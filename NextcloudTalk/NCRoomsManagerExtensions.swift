        ...
        self.joinRoomTask = NCAPIController.sharedInstance().joinRoom(token, for: activeAccount, withCompletionBlock: { sessionId, room, error, statusCode in
            if !self.isJoiningRoom(withToken: token) {
                ...
                if let error = error as? NSError, error.code != NSURLErrorCancelled {
                    ...
                    completionBlock(nil, nil, nil, NCRoomsManager.statusCodeIgnoreJoinAttempt)
                } else {
                    NCUtils.log("Not joining the room any more, but our join request was successful.")
                    completionBlock(nil, nil, nil, NCRoomsManager.statusCodeShouldIgnoreAttemptButJoinedSuccessfully)
                }
